...
This library proves types and refuses to grow into a validator, but the 422 shape is a perfectly
good transport for the rules it will not check for you:

```php
throw InvalidInputException::createFromMessages(['email' => ['Already taken']]);
```

That produces a 422 from any field-to-message map. Throw it from a handler or a middleware and the
client reads it exactly like a type failure — same branch, same `details.fields`.
