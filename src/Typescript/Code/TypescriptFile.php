...
final readonly class TypescriptFile implements Stringable
{
    ...
    /**
     * Written as the first line of every generated file, and the only thing that identifies one.
     * OutputDirectory deletes what it finds carrying this and nothing else, so hand written
     * TypeScript can share the output directory without being destroyed by a regeneration.
     *
     * Deliberately free of a version, a timestamp and a path: generated files are compared byte for
     * byte to decide whether they are stale, and anything varying would report every file as
     * changed on every run.
     */
    public const string MARKER = '// generated by: php-ts-bindings';

    /**
     * Whether $contents was written by this library. Reads the first line only, so it can be asked
     * of a file on disk without loading it whole.
     */
